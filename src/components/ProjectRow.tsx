...
export default function ProjectRow({
  ...
  // Determine the primary link destination: demo URL if available, otherwise project blog page
  const primaryLink = hasDemo ? demoUrl! : `/projects/${slug}`;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
        ...
        <div className="order-2 lg:order-1">
          <Link href={primaryLink} className="block">
            <div className="relative w-full overflow-hidden rounded-xl bg-alabaster cursor-pointer hover:opacity-90 transition-opacity">
              {thumbnail ? (
                <Image
                  src={thumbnail}
                  alt={`${title} thumbnail`}
          ...
          <header>
            <div className="flex items-center gap-3 mb-2">
              <h3 className="text-2xl font-bold text-black">
                <Link href={primaryLink} className="hover:text-davy-gray transition-colors">
                  {title}
                </Link>
              </h3>
